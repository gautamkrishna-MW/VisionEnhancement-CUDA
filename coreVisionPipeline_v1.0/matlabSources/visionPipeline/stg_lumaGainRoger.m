...
function [outFrameLeft, outFrameRight] = stg_lumaGainRoger(frameLeft, frameRight)
%#codegen
...
for i = 1:2 % Dummy Kernel call
    while(integrateHist(binVal) < percentVal(1)*numPixels && binVal < numBins)
        binVal = binVal + 1;
        bin1Percent = binVal;
    end
    binVal = numBins;
    while (integrateHist(binVal) >= percentVal(2)*numPixels && binVal >= 0)
        bin99Percent = binVal;
        binVal = binVal - 1;
    end
end
...
for iterVal = 0:numBins-1
    tmpVal = (iterVal - bin1Percent);
    if tmpVal < 0
        tmpVal = 0;
    end
    pixelGainLUT(iterVal+1) = (tmpVal*4095)/(bin99Percent - bin1Percent);
end
