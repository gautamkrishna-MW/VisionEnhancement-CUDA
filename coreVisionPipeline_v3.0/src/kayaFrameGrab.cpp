...
void kayaFrameGrabber::setupVirtualCamerasAndRegisterCallback()
{
    for (int camIter = 0; camIter < numCamerasDetected; camIter++)
    {
        ...
        {
            uint16_t* pBuffer = processFrameObjPtr->frameBufferArray[camIter*MAXBUFFERS + iFrame];
            LOG("Frame " << iFrame << " Pointer : " << pBuffer);
            kayaErrchk(KYFG_BufferAnnounce(camStreamHandle[camIter], (void*)pBuffer, frameDataSize, NULL, &streamBufferHandle[camIter][iFrame]));
        }
    }
...
void kayaFrameGrabber::runAcquisitionLoopUntilStop()
{
    ...
    while(!stopLoop)
    {
        ...
        if (tmpVal != AWBRegisterValue)
        {
            ...
            this->processFrameObjPtr->runAWB = true;
            sleep(2);
            while (this->isLeftFrameSaturated || this->isRightFrameSaturated)
            {
                this->isLeftFrameSaturated = false; this->isRightFrameSaturated = false;
            ...
            this->processFrameObjPtr->runAWB = false;
        }
    }
}
...
void kayaFrameGrabber::rightCameraCallback(STREAM_BUFFER_HANDLE streamBufferHandle, void* userContext)
{
    ...
    /* Process frame */
    // LOG("Right Frame: " << "\n\tFramePtr: " << frMemPtr << "\n\tFrameID: " << frameId);
    LOG("AWBRegisterValue: " << fgObj->AWBRegisterValue);
    ...
    if (fgObj->processFrameObjPtr != nullptr)
    {
        if (fgObj->processFrameObjPtr->runAWB)
        {
            ...
            LOG("AWB on Right Frame: " << fgObj->isRightFrameSaturated);
        }
        else
        {
...
void kayaFrameGrabber::leftCameraCallback(STREAM_BUFFER_HANDLE streamBufferHandle, void* userContext)
{
    ...
    if (fgObj->processFrameObjPtr != nullptr)
    {
        if (fgObj->processFrameObjPtr->runAWB)
        {
            ...
            LOG("AWB on Left Frame: " << fgObj->isLeftFrameSaturated);
        }
        else
        {
